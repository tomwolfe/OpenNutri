...
import { Camera, Upload, X, CheckCircle, AlertCircle, Loader2, Edit2, Save, WifiOff, Cloud, Barcode, Mic } from 'lucide-react';
...
export function SnapToLog({ onComplete, onError, onDraftSaved, onSyncComplete }: SnapToLogProps) {
  ...
  const handleProductFound = (item: DraftItem) => {
    setDraftItems([item]);
    setUploadProgress('review');
    setMode('vision'); // Switch back to vision mode to show review UI
  };
  ...
  const startListening = useCallback(() => {
    ...
    recognition.onerror = (event: { error: string }) => {
      ...
      setUploadError('Failed to recognize speech. Please try again.');
    };
    ...
    try {
      ...
      if (isReady) {
        try {
          ...
          const encryptionResult = await encryptLog(draftItems as unknown as LogItem[]);
    ...
    <div className="w-full max-w-md mx-auto p-4 bg-white rounded-lg shadow-sm border">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-gray-500">
          Take a photo or scan a barcode for instant nutrition analysis
        </p>
      </div>
      ...
      {/* Barcode Mode */}
      {mode === 'barcode' && uploadProgress === 'idle' ? (
        <BarcodeScanner 
          onProductFound={handleProductFound}
          onClose={() => setMode('vision')}
        />
      ) : mode === 'voice' && uploadProgress === 'idle' ? (
        /* Voice Mode */
        <div className="flex flex-col items-center justify-center py-8 gap-4 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
          <div className={`p-6 rounded-full transition-all cursor-pointer ${isListening ? 'bg-red-100 animate-pulse' : 'bg-blue-100'}`}
