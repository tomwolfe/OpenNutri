...
export async function POST(request: NextRequest) {
  try {
    ...
    const aiItems = items.map((item) => ({
      ...
      fat_g: item.fat || 0,
      fiber_g: item.micronutrients?.fiber,
      sugar_g: item.micronutrients?.sugar,
      sodium_mg: item.sodium || item.micronutrients?.sodium,
      potassium_mg: item.micronutrients?.potassium,
      calcium_mg: item.micronutrients?.calcium,
      iron_mg: item.micronutrients?.iron,
      vitamin_c_mg: item.micronutrients?.vitaminC,
