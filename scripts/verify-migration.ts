...
async function verifyMigration() {
  ...
  const pkResult: any = await sql.query(`
  ...
  const columnsResult: any = await sql.query(`
  ...
  console.log('   Columns:', columns.map((r: any) => r.column_name));

  // Check if ai_jobs table exists
  console.log('\n3. Checking if ai_jobs table exists (should be dropped)...');
  const tableExists: any = await sql.query(`
    SELECT table_name FROM information_schema.tables 
    WHERE table_name = 'ai_jobs';
  `);
  const tables = Array.isArray(tableExists) ? tableExists : tableExists?.rows || [];
